...
export const actions = {
    async nuxtServerInit({ commit }) {
        try {
            const redisUrl = `${process.env.CLIENT_URL}/api/spotify/data/`
            const { data: { is_connected } } = await axios.get(`${redisUrl}is_connected`)

            commit('connectionChange', is_connected)
            if (Boolean(is_connected)) {
                const { data: { item, is_playing } } = await axios.get(`${process.env.CLIENT_URL}/api/spotify/now-playing`)
